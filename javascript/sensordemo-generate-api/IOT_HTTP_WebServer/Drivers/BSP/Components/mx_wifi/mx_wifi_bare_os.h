...
#define LOCK_DECLARE(A)                           volatile bool (A)
#define LOCK_INIT(A)                              (A) = false
#define LOCK_DEINIT(A)
#define LOCK(A)                                   while(A){}; (A) = true
#define UNLOCK(A)                                 (A) = false

#define SEM_DECLARE(A)                            volatile uint32_t (A)
#define SEM_INIT(A,COUNT)                         (A) = 0
#define SEM_DEINIT(A)
#define SEM_SIGNAL(A)                             ((A)++)
#define SEM_WAIT(A,TIMEOUT,IDLE_FUNC)             noos_sem_wait(&(A), (TIMEOUT), (IDLE_FUNC))

#define THREAD_DECLARE(A)
#define THREAD_INIT(A, THREAD_FUNC, THREAD_CONTEXT, STACKSIZE, PRIORITY)  0
#define THREAD_DEINIT(A)                          (void) 0
#define THREAD_TERMINATE()
#define THREAD_CONTEXT_TYPE                       void*

#define FIFO_DECLARE(QUEUE)                       noos_queue_t* (QUEUE)
#define FIFO_INIT(QUEUE, QSIZE)                   noos_fifo_init(&(QUEUE), (QSIZE))
#define FIFO_PUSH(QUEUE, VALUE, TIMEOUT, IDLE_FUNC) \
  noos_fifo_push((QUEUE), (VALUE), (TIMEOUT), (IDLE_FUNC))
#define FIFO_POP(QUEUE, TIMEOUT, IDLE_FUNC)       noos_fifo_pop((QUEUE), (TIMEOUT), (IDLE_FUNC))
#define FIFO_DEINIT(QUEUE)                        noos_fifo_deinit((QUEUE))
